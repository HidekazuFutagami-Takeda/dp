 ...
 * @author tkawabata
 */
public class InsMst extends Model<InsMst> {
...
// add end 2020/03/25 趙 2020年4月組織変更対応（各種アプリ）
//add Start 2025/04/07 H.Futagami 施設検索の担当者表示の変更
		if(getBu2csMrJgiNo() != null && getBu2csMrJgiNo() != 0){
			JgiMst bu2cs = new JgiMst();
			bu2cs.setJgiNo(getBu2csMrJgiNo());
			bu2cs.setJgiName(getBu2csMrJgiName());
			//bu2cs.setShokushuName(getKokeiShokushuName());
			tantoList.add(bu2cs);
		}

		if(getJohlcMrJgiNo() != null && getJohlcMrJgiNo() != 0){
			JgiMst johlc = new JgiMst();
			johlc.setJgiNo(getJohlcMrJgiNo());
			johlc.setJgiName(getJohlcMrJgiName());
			//bu2cs.setShokushuName(getKokeiShokushuName());
			tantoList.add(johlc);
		}

		if(getBu2bloMrJgiNo() != null && getBu2bloMrJgiNo() != 0){
			JgiMst bu2blo = new JgiMst();
			bu2blo.setJgiNo(getBu2bloMrJgiNo());
			bu2blo.setJgiName(getBu2bloMrJgiName());
			//bu2cs.setShokushuName(getKokeiShokushuName());
			tantoList.add(bu2blo);
		}

		if(getBu2herMrJgiNo() != null && getBu2herMrJgiNo() != 0){
			JgiMst bu2her = new JgiMst();
			bu2her.setJgiNo(getBu2herMrJgiNo());
			bu2her.setJgiName(getBu2herMrJgiName());
			//bu2cs.setShokushuName(getKokeiShokushuName());
			tantoList.add(bu2her);
		}

		if(getJoguhMrJgiNo() != null && getJoguhMrJgiNo() != 0){
			JgiMst joguh = new JgiMst();
			joguh.setJgiNo(getJoguhMrJgiNo());
			joguh.setJgiName(getJoguhMrJgiName());
			//bu2cs.setShokushuName(getKokeiShokushuName());
			tantoList.add(joguh);
		}

		if(getBu1lamMrJgiNo() != null && getBu1lamMrJgiNo() != 0){
			JgiMst bu1lam = new JgiMst();
			bu1lam.setJgiNo(getBu1lamMrJgiNo());
			bu1lam.setJgiName(getBu1lamMrJgiName());
			//bu2cs.setShokushuName(getKokeiShokushuName());
			tantoList.add(bu1lam);
		}
//add End 2025/04/07 H.Futagami 施設検索の担当者表示の変更
...
// add end   2018/05/21 S.Hayashi J18-0002_2018年4月組織変更対応（計画支援）
	...
	@Override
	public int compareTo(InsMst obj) {
		if (obj != null) {
	...
	public boolean equals(Object entry) {
		if (entry != null && InsMst.class.isAssignableFrom(entry.getClass())) {
			InsMst obj = (InsMst) entry;
