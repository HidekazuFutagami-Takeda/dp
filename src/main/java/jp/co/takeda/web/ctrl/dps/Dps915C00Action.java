...
import jp.co.takeda.web.in.dps.Dps915C00Form;
...
/**
 * Dps915C00Action(施設主担当従業員検索_MR編集)のアクションクラス
 * 
 * @author hashidume
 */
@Controller("Dps915C00Action")
public class Dps915C00Action extends DpAction {

	/**
	 * LOGGER
	 */
	private static final Log LOG = LogFactory.getLog(Dps915C00Action.class);
	...
	public Result dps915C00F00(DpContext ctx, Dps915C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("Dps915C00F00");
		...
		// 検索サービス実行
		List<JgiMst> listDto = dpsSosJgiSearchService.searchTlMrList(jgiNo);
		
		// 結果格納DTOをリクエストに格納
		super.getRequestBox().put(Dps915C00Form.SOS_JGI_LIST_DTO_KEY_R, (ArrayList<JgiMst>)listDto);
	...
	public void dps915C00F00Validation(DpContext ctx, Dps915C00Form form) throws Exception {
		// 従業員番号 必須チェック
