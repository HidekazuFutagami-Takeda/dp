...
import jp.co.takeda.web.in.dpm.Dpm601C01Form;

/**
 * Dpm601C01((ワ)施設品目別計画編集画面)のアクションクラス
 *
 * @author stakeuchi
 */
@Controller("Dpm601C01Action")
public class Dpm601C01Action extends DpAction {

	/**
	 * LOGGER
	 */
	private static final Log LOG = LogFactory.getLog(Dpm601C01Action.class);

	/**
	 * 画面ID
	 */
	private static final String SCREEN_ID = "DPM601C01";
	...
	public Result dpm601C01F00(DpContext ctx, Dpm601C01Form form) throws Exception {

		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm601C01F00");
		}
	...
	@RequestType(businessType = VACCINE)
	@Permission(authType = REFER)
	public Result dpm601C01F05(DpContext ctx, Dpm601C01Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm601C01F05");
		}
		form.setTranField();
	...
	@RequestType(businessType = VACCINE)
	@Permission(authType = REFER)
	public Result dpm601C01F10Execute(DpContext ctx, Dpm601C01Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm601C01F10");
		}
	 ...
	 * @throws Exception 例外
	 */
	private void searchInsPlanForVacList(DpContext ctx, Dpm601C01Form form) throws Exception {
		...
		try {
			...
			super.getRequestBox().put(Dpm601C01Form.DPM601C01_DATA_R_SEARCH_RESULT, resultDto);

		} finally {

			// ヘッダー情報取得
			InsPlanForVacHeaderDto headerDto = dpmInsPlanForVacSearchService.searchInsPlanHeader(scDto.getInsNo());
			super.getRequestBox().put(Dpm601C01Form.DPM601C01_DATA_R_HEADER, headerDto);
	 ...
	 * @param form ActionForm
	 */
	private void setUserInfo(Dpm601C01Form form) {
...
//		} else if (dpUser.isMatch(JokenSet.TOKUYAKUTEN_G_MASTER, JokenSet.TOKUYAKUTEN_G_STAFF, JokenSet.TOKUYAKUTEN_GYOUMU_G, JokenSet.WAKUTIN_AL)) {
	...
	public void dpm601C01F05Validation(DpContext ctx, Dpm601C01Form form) throws ValidateException {
		// [必須] 施設コード
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm601C01F10Validation(DpContext ctx, Dpm601C01Form form) throws ValidateException {
		// 更新パラーメータチェック
