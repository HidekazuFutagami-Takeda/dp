...
import jp.co.takeda.web.in.dps.Dps202C00Form;

/**
 * Dps202C00((医)特定施設個別計画施設一覧画面(営業所案))のアクションクラス
 *
 * @author stakeuchi
 */
@Controller("Dps202C00Action")
public class Dps202C00Action extends DpAction {

	/**
	 * LOGGER
	 */
	private static final Log LOG = LogFactory.getLog(Dps202C00Action.class);

	/**
	 * 画面ID
	 */
	private static final String SCREEN_ID = "DPS202C00";
	...
	public Result dps202C00F00(DpContext ctx, Dps202C00Form form) throws Exception {

		if (LOG.isDebugEnabled()) {
			LOG.debug("dps202C00F00");
		}
	...
	@RequestType
	@Permission(authType = REFER)
	public Result dps202C00F05(DpContext ctx, Dps202C00Form form) throws Exception {

		if (LOG.isDebugEnabled()) {
			LOG.debug("dps202C00F05");
	...
	@RequestType
	@Permission(authType = REFER)
	public Result dps202C00F25(DpContext ctx, Dps202C00Form form) throws Exception {
	...
	public Result dps202C00F10Execute(DpContext ctx, Dps202C00Form form) throws Exception {
		...
		try {
			...
			// 削除実行
			dpsSpecialInsPlanService.deleteSpecialInsPlan(deleteDtoList, null, "1".equals(form.getVaccine()));
	...
	@RequestType
	@Permission(authType = REFER)
	public Result dps202C00F15(DpContext ctx, Dps202C00Form form) throws Exception {
	...
	@RequestType
	@Permission(authType = REFER)
	public Result dps202C00F16(DpContext ctx, Dps202C00Form form) throws Exception {
	 ...
	 * @throws Exception 例外
	 */
	private void searchSpecialInsPlan(Dps202C00Form form) throws Exception {
		...
		try {
			...
			resultList = dpsSpecialInsPlanSearchService.searchSpecialInsPlan(scDto, PlanType.OFFICE);
		...
		// リクエストに検索結果リストをセット
		super.getRequestBox().put(Dps202C00Form.DPS202C00_DATA_R, (ArrayList<SpecialInsPlanResultDto>) resultList);
	}
	 ...
	 * @throws Exception 例外
	 */
	private void searchSpecialInsPlanVac(Dps202C00Form form) throws Exception {
		...
		try {
			...
			resultList = dpsSpecialInsPlanForVacSearchService.searchSpecialInsPlanForVac(scDto, PlanType.OFFICE);
			form.setExistSearchDataFlag(true);
		...
		super.getRequestBox().put(Dps202C00Form.DPS202C01_DATA_R, (ArrayList<SpecialInsPlanForVacResultDto>) resultList);
	}
	 ...
	 * @throws Exception 例外
	 */
	private void searchAddrCodeCity(Dps202C00Form form) throws Exception {
...
//		if (form.getPrefectures() != null) {
		...
		// リクエストBOXに検索結果リストをセット
		super.getRequestBox().put(Dps202C00Form.DPS202C00_DATA_ADDR_CODE_CITY_LIST, (ArrayList<CodeAndValue>) resultList);
	}
	 ...
	 * @throws Exception 例外
	 */
	private void searchPrefectures(Dps202C00Form form) throws Exception {
		...
		super.getRequestBox().put(Dps202C00Form.DPS202C00_DATA_PREFECTURE_LIST, (ArrayList<CodeAndValue>) resultList);
	}
	...
	public void dps202C00F05Validation(DpContext ctx, Dps202C00Form form) throws ValidateException {
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dps202C00F25Validation(DpContext ctx, Dps202C00Form form) throws ValidateException {
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dps202C00F10Validation(DpContext ctx, Dps202C00Form form) throws ValidateException {
