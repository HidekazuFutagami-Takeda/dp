...
import jp.co.takeda.web.in.dpm.Dpm200C01Form;

/**
 * Dpm200C01((ワ)施設別計画編集画面)のアクションクラス
 *
 * @author stakeuchi
 */
@Controller("Dpm200C01Action")
public class Dpm200C01Action extends DpAction {

	/**
	 * LOGGER
	 */
	private static final Log LOG = LogFactory.getLog(Dpm200C01Action.class);

	/**
	 * 画面ID
	 */
	private static final String SCREEN_ID = "DPM200C01";
	...
	public Result dpm200C01F00(DpContext ctx, Dpm200C01Form form) throws Exception {

		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm200C01F00");
		}
	...
	@RequestType(businessType = VACCINE)
	@Permission(authType = REFER)
	public Result dpm200C01F05(DpContext ctx, Dpm200C01Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm200C01F05");
		}
	...
	public Result dpm200C01F10Execute(DpContext ctx, Dpm200C01Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm200C01F10");
		}
	 ...
	 * @throws Exception 例外
	 */
	private void searchInsPlanForVacList(Dpm200C01Form form) throws Exception {
		...
		try {
			...
			super.getRequestBox().put(Dpm200C01Form.DPM200C01_DATA_R_SEARCH_RESULT, resultDto);
		} finally {

			// 集計行取得
			InsPlanForVacResultDetailTotalDto totalDto = dpmInsPlanForVacSearchService.searchInsPlanTotal(scDto, resultDto);
			super.getRequestBox().put(Dpm200C01Form.DPM200C01_DATA_R_SEARCH_RESULT_TOTAL, totalDto);
	 ...
	 * @param form ActionForm
	 */
	private void setUserInfo(Dpm200C01Form form) {
...
//		} else if (dpUser.isMatch(JokenSet.TOKUYAKUTEN_G_MASTER, JokenSet.TOKUYAKUTEN_G_STAFF, JokenSet.TOKUYAKUTEN_GYOUMU_G, JokenSet.WAKUTIN_AL)) {
	...
	public void dpm200C01F05Validation(DpContext ctx, Dpm200C01Form form) throws ValidateException {
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm200C01F10Validation(DpContext ctx, Dpm200C01Form form) throws ValidateException {
		// 更新パラーメータチェック
