...
import jp.co.takeda.service.DpmIyakuMonthPlanSearchService;
import jp.co.takeda.service.DpmIyakuMonthPlanService;
...
import jp.co.takeda.web.in.dpm.Dpm501C00Form;

/**
 * Dpm501C00((医)品目別計画編集画面)のアクションクラス
 *
 * @author stakeuchi
 */
@Controller("Dpm501C00Action")
public class Dpm501C00Action extends DpAction {

	/**
	 * LOGGER
	 */
	private static final Log LOG = LogFactory.getLog(Dpm501C00Action.class);

	/**
	 * 画面ID
	 */
	private static final String SCREEN_ID = "DPM501C00";
	 ...
	 */
	@Autowired(required = true)
	@Qualifier("dpmIyakuMonthPlanService")
	protected DpmIyakuMonthPlanService dpmIyakuMonthPlanService;
	...
	public Result dpm501C00F00(DpContext ctx, Dpm501C00Form form) throws Exception {

		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F00");
		}
	...
	@RequestType
	@Permission(authType = AuthType.REFER)
	public Result dpm501C00F01(DpContext ctx, Dpm501C00Form form) throws Exception {

		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F01");
	...
	@RequestType
	@Permission(authType = AuthType.REFER)
	public Result dpm501C00F05(DpContext ctx, Dpm501C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F05");
			LOG.debug("SosCd2:" + form.getSosCd2());
			LOG.debug("SosCd3:" + form.getSosCd3());
			LOG.debug("SosCd4:" + form.getSosCd4());
	...
	public Result dpm501C00F10Execute(DpContext ctx, Dpm501C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F10Execute");
			LOG.debug("SosCd2:" + form.getSosCd2());
			LOG.debug("SosCd3:" + form.getSosCd3());
			LOG.debug("SosCd4:" + form.getSosCd4());
			LOG.debug("ProdCategory:" + form.getProdCategory());
		...
		try {
			...
			if (!updateDtoList.isEmpty()) {
				result = dpmIyakuMonthPlanService.updateIyakuPlan(SCREEN_ID, updateDtoList);
			}
	...
	@RequestType
	@Permission(authType = AuthType.EDIT)
	public Result dpm501C00F11Execute(DpContext ctx, Dpm501C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F11Execute");
			LOG.debug("SosCd2:" + form.getSosCd2());
			LOG.debug("SosCd3:" + form.getSosCd3());
			LOG.debug("SosCd4:" + form.getSosCd4());
			LOG.debug("ProdCategory:" + form.getProdCategory());
	...
	@RequestType
	@Permission(authType = AuthType.EDIT)
	public Result dpm501C00F12Execute(DpContext ctx, Dpm501C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F12Execute");
			LOG.debug("SosCd2:" + form.getSosCd2());
			LOG.debug("SosCd3:" + form.getSosCd3());
			LOG.debug("SosCd4:" + form.getSosCd4());
			LOG.debug("ProdCategory:" + form.getProdCategory());
	...
	@RequestType
	@Permission(authType = AuthType.EDIT)
	public Result dpm501C00F14Execute(DpContext ctx, Dpm501C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F13Execute");
			LOG.debug("SosCd2:" + form.getSosCd2());
			LOG.debug("SosCd3:" + form.getSosCd3());
			LOG.debug("SosCd4:" + form.getSosCd4());
			LOG.debug("ProdCategory:" + form.getProdCategory());
	 ...
	 * @throws Exception 例外
	 */
	private void searchProdPlanList(Dpm501C00Form form) throws Exception {
		...
		try {
			...
			// リクエストボックスに検索結果をセット
			super.getRequestBox().put(Dpm501C00Form.DPM501C00_DATA_R_SEARCH_RESULT, resultDto);
			...
			super.getRequestBox().put(Dpm501C00Form.DPM501C00_DATA_R_BUMON_FLAG, bumonFlag);
	 ...
	 * @param form ActionForm
	 */
	private void initCategoryList(Dpm501C00Form form) {
		// 汎用マスタのカテゴリリスト
	 ...
	 * @param form
	 */
	private void setGridHeader(Dpm501C00Form form) {
...
// mod End 2022/08/25 R.takamoto No.8　計画管理の月別計画に納入実績の値を表示
	 ...
	 *
	 * @param form Dpm501Form
	 * @return JRNSに含まれるカテゴリリスト
	 */
	private List<String> createJrnsCategoryList(Dpm501C00Form form) {
		// JRNSに含まれるカテゴリリスト
	...
	public void dpm501C00F05Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		// [必須] 品目カテゴリ
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm501C00F10Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm501C00F11Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		executeValidation(ctx, form);
	}
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm501C00F12Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		executeValidation(ctx, form);
	}
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm501C00F13Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		executeValidation(ctx, form);
	}
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm501C00F14Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		executeValidation(ctx, form);
	}
	 ...
	 * @throws Exception 例外(ValidateException)
	 */
	private void executeValidation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		// 更新パラーメータチェック
		...
		for (int i = 0; i < rowIdList.length; i++) {
			...
			// 更新後月末見込6
			if (StringUtils.isNotEmpty(rowId[29]) && !ValidationUtil.isLong(rowId[29])) {
	...
	public Result dpm501C00F15Output(DpContext ctx, Dpm501C00Form form) throws Exception {
		if (LOG.isDebugEnabled()) {
			LOG.debug("dpm501C00F15Output");
		}
		...
		//ExportResult exportResult = dpmReportService.outputProdMonthPlanList(templateRootPath, prodPlanScDto, jrnsCtgList);
		ExportResult exportResult = dpmReportService.outputProdMonthPlanList(templateRootPath, prodPlanScDto, jrnsCtgList, form.getTougetuCount());
	 ...
	 */
	@ActionMethod(methodType = MethodType.VALIDATION)
	public void dpm501C00F15Validation(DpContext ctx, Dpm501C00Form form) throws ValidateException {
		dpm501C00F05Validation(ctx, form);
	}
