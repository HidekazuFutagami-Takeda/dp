...
/**
 * 配分ロジッククラス<br>
 * 配分品目の納入実績による構成比配分を行なう。
 *
 * @author nozaki
 */
public class DistributionByRateLogic extends DistributionBaseLogic {

	/**
	 * ロガー
	 */
	private static final Log LOG = LogFactory.getLog(DistributionByRateLogic.class);
	 ...
	 */
	public DistributionByRateLogic(List<DistributionElement> elementList, DistParam distParam, String sosCd, Integer jgiNo, String prodCode, InsType insType, Long paramValue,
		Long specialSum, InsWsDistType distType) {
		super(distType);
...
//			} else if (element.getExceptDistInsFlg()) {
			if (element.getExceptDistInsFlg()) {
				...
				// 配分品目実績合計
				kakoJissekiSum = MathUtil.add(kakoJissekiSum, element.getSumResult());
			}
		}
		...
		if (distParam.getZeroDistFlg()) {
			...
			// (3)(4)配分母数、および、納入実績構成比から、配分値を算出 (戻り値は配分値合計)
			Long distValueSum = createInsWsPlan(insWsPlanList, prodCode, distList, InsWsPlanType.NORMAL, distValue, kakoJissekiSum, null);
