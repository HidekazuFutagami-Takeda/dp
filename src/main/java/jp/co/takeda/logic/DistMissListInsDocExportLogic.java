...
/**
 * (医)施設特約店別計画配分ミスリスト(担当者計画)のExcelファイルを生成するロジッククラス
 *
 * @author stakeuchi
 */
public class DistMissListInsDocExportLogic {
	 ...
	 * 対象計画名
	 */
	private static final String PLAN_NAME = "施設医師別計画";
	...
	// [列番号] 印刷終了列
	private static final int COL_IDX_PRINT_END = 7;
	 ...
	 */
	public DistMissListInsDocExportLogic(String templatePath, Date systemDate, OutputFile outputFile, List<MrDistMiss> mrDistMissList, MessageSource messageSource) {
		// ----------------------
		// 引数チェック
		// ----------------------
	...
	private void writeListInfo(POIBean poiBean) {
		...
		for (MrDistMiss mrDistMiss : mrDistMissList) {
			...
			// チーム
			final String teamName = mrDistMiss.getBumonRyakuName();
			poiBean.setCellData(teamName, rowIdx, colIdx++);

			// 担当者
			String jgiName = mrDistMiss.getJgiName();
			final String shokushuName = mrDistMiss.getShokushuName();
			if(StringUtils.isNotEmpty(shokushuName)){
				jgiName = jgiName + "（" + shokushuName + "）";
			}
			...
			// 対象区分
			final String insType = mrDistMiss.getInsType().toString();
