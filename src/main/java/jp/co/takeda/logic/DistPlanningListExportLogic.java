 ...
 * @author khashimoto
 */
public class DistPlanningListExportLogic {
	 ...
	 */
	private final List<DistPlanningListDLSummary> distPlanningListDLSummaryList;
...
// mod End   2022/5/20 R.Yoshida 2022年4月組織変更対応（支店→リージョン、営業所→エリア）
	 ...
	 */
	public DistPlanningListExportLogic(String templatePath, Date systemDate, List<DistPlanningListDLSummary> distPlanningListDLSummaryList) {
		this.templatePath = templatePath;
		this.systemDate = (Date) systemDate.clone();
		this.distPlanningListDLSummaryList = distPlanningListDLSummaryList;
...
//		return OUTPUT_FILE_NAME_HEADER + "_" + DateUtil.toString(systemDate, "yyyyMMddHHmmss") + ".xls";
	...
	private void writeListInfo(POIBean poiBean) {
		...
		if (distPlanningListDLSummaryList == null || distPlanningListDLSummaryList.isEmpty()) {
			return;
		}
		...
		// add Start 2023/2/8  Y.Taniguchi フォント出力修正
		//3行目からフォント形式が変わるため1行目のセル情報を保持
		XSSFCellStyle style = poiBean.createCellStyle();
		XSSFRow row = poiBean.getWorkSheet().getRow(1);
		XSSFCell cell = row.getCell(1);
		style.setFont(cell.getCellStyle().getFont());
		style.setDataFormat(cell.getCellStyle().getDataFormat());
		// add End 2023/2/8  Y.Taniguchi フォント出力修正

		for (DistPlanningListDLSummary distPlanningSummary : distPlanningListDLSummaryList) {
			...
			// mod Start 2023/2/8  Y.Taniguchi フォント出力修正
//			poiBean.setCellData(brDistCode, rowIdx, colIdx++);
//			poiBean.setCellData(brDistName, rowIdx, colIdx++);
			poiBean.setCellData(brDistCode, rowIdx, colIdx++, style);
			poiBean.setCellData(brDistName, rowIdx, colIdx++, style);
			// mod End 2023/2/8  Y.Taniguchi フォント出力修正

			// 統計品目コード
			final String prodCode = distPlanningSummary.getStatProdCd();
			// mod Start 2023/2/8  Y.Taniguchi フォント出力修正
//			poiBean.setCellData(prodCode, rowIdx, colIdx++);
			poiBean.setCellData(prodCode, rowIdx, colIdx++, style);
			// mod End 2023/2/8  Y.Taniguchi フォント出力修正
...
//			poiBean.setCellData(prodName, rowIdx, colIdx++);
			poiBean.setCellData(prodName, rowIdx, colIdx++, style);
			// mod End 2023/2/8  Y.Taniguchi フォント出力修正
		...
		// 印刷範囲を設定
		final int rowIdxPrintEnd = ROW_IDX_START_LIST + distPlanningListDLSummaryList.size();
