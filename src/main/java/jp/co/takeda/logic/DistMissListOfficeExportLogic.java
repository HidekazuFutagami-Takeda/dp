...
/**
 * (医)施設特約店別計画配分ミスリスト(営業所計画)のExcelファイルを生成するロジッククラス
 *
 * @author stakeuchi
 */
public class DistMissListOfficeExportLogic {
...
// mod Start 2022/5/20 R.Yoshida 2022年4月組織変更対応（支店→リージョン、営業所→エリア）
	private static final String PLAN_NAME = "施設特約店別計画配分（エリア計画）";
//	private static final String PLAN_NAME = "施設特約店別計画配分（営業所計画）";
// mod End 2022/5/20 R.Yoshida 2022年4月組織変更対応（支店→リージョン、営業所→エリア）
	 ...
	 */
	public DistMissListOfficeExportLogic(String templatePath, Date systemDate, OutputFile outputFile, List<SosDistMiss> sosDistMissList, MessageSource messageSource) {
		// ----------------------
		// 引数チェック
		// ----------------------
	...
	private void writeListInfo(POIBean poiBean) {
		...
		// リストのサイズ分行を追加
		poiBean.addRows(ROW_IDX_START_LIST, sosDistMissList.size());
		...
		// 印刷範囲を設定
		final int rowIdxPrintEnd = ROW_IDX_START_LIST + sosDistMissList.size();
