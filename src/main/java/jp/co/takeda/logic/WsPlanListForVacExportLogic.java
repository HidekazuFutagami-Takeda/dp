...
/**
 * (ワ)特約店別計画一覧のExcelファイルを生成するロジッククラス
 *
 * @author stakeuchi
 */
public class WsPlanListForVacExportLogic {
	 ...
	 */
	public WsPlanListForVacExportLogic(String templatePath, Date systemDate, WsPlanReferenceForVacScDto refScDto, List<WsPlanForVacSummary2> summaryList,
...
//	private void write(HSSFWorkbook workBook) {
	...
	private void writeListInfo(POIBean poiBean) {
		...
		for (WsPlanForVacSummary2 summary : summaryList) {
			...
			// 前回と特約店情報が異なる場合のみ表示
			final String currentWordTyten = tytenCode + tytenName;
			if (!StringUtils.equals(workWordTyten, currentWordTyten)) {
			...
			workWordTyten = currentWordTyten; // 現在の特約店情報を保持
			...
			MonNnuSummary monNnuSummary = summary.getMonNnuSummary(); // 納入実績
			// 前々々期
			...
			// 前同比
			final String ratioFormula = createRatioFormula(poiBean, rowIdx, colIdx);
			poiBean.setCellFormula(ratioFormula, rowIdx, colIdx++);
