...
/**
 * お知らせ対象の従業員を取得するロジッククラス
 *
 * @author khashimoto
 */
public class SearchAnnounceJgiListLogic {
	 ...
	 */
	public SearchAnnounceJgiListLogic(SosMstDAO sosMstDAO, JgiMstDAO jgiMstDAO, DpUser dpUser, ContactOperationsType operationsType, String sosCd) {
	...
	/**
	 * (医)試算処理/施設特約店別配分処理(MMP品)のお知らせ先を取得する。
	 *
	 * @return 従業員リスト
	 * @throws DataNotFoundException
	 */
	protected List<JgiMst> searchMMPJgiList() throws DataNotFoundException {
		List<JgiMst> jgiMstList = new ArrayList<JgiMst>();
		// 本部、支店長、支店スタッフは実行者に通知
		if (dpUser.isMatch(JokenSet.HONBU, JokenSet.SITEN_MASTER, JokenSet.SITEN_STAFF)) {
