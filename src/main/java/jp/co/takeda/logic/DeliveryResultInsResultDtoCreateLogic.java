...
/**
 * 過去実績参照(施設特約店別計画)ロジッククラス
 *
 * @author siwamoto
 */
public class DeliveryResultInsResultDtoCreateLogic {

	/**
	 * 納入実績リスト
	 */
	List<DeliveryResult> deliveryResultList;
	 ...
	 */
	public DeliveryResultInsResultDtoCreateLogic(List<DeliveryResult> deliveryResultList, String jgiName, String shokushuName) {
	 ...
	 * @param jgiName 従業員名
	 */
	public DeliveryResultInsResultDtoCreateLogic(String jgiName, String shokushuName) {
		this.jgiName = jgiName;
		this.shokushuName = shokushuName;
	}
	 ...
	 * @return 過去実績詳細検索結果
	 */
	public DeliveryResultInsListDto createResultDtoList() throws LogicalException {
		...
		for (DeliveryResult deliveryResult : deliveryResultList) {
			//add Start 2022/5/11 H.Futagami バックログNo.13　過去実績参照で選択している品目を一覧に表示する
			prodName = deliveryResult.getProdName();
			//add End 2022/5/11 H.Futagami バックログNo.13　過去実績参照で選択している品目を一覧に表示する
			if (!deliveryResult.getInsNo().equals(oldInsNo) && !isFirst) {
				...
				DeliveryResultInsDto resultDto = new DeliveryResultInsDto(insName, "計", monNnuValueList, fontColor, null);
			...
			// 品目施設情報の取得
			fontColor = deliveryResult.getProdIns().getDispFontColCd();
			if (deliveryResult.getProdIns().getInsInfoName() != null) {
				insInfoName = "　" + deliveryResult.getProdIns().getInsInfoName();
			} else {
				insInfoName = "";
			}
			String insName = "";
			...
			DeliveryResultInsDto resultDto = new DeliveryResultInsDto(insName, tmsTytenName, monNnuValueList, fontColor, deliveryResult.getProdIns().getProdInsInfoErrKbn());
			resultDtoList.add(resultDto);
		...
		//DeliveryResultInsListDto result = new DeliveryResultInsListDto(resultDtoList, refDateList, jgiName, shokushuName);
		DeliveryResultInsListDto result = new DeliveryResultInsListDto(resultDtoList, refDateList, jgiName, shokushuName, prodName);
	 ...
	 * @return 過去実績詳細検索結果
	 */
	public DeliveryResultInsListDto createResultDtoList2() throws LogicalException {
		String sysYear = DpUserInfo.getDpUserInfo().getSysManage().getSysYear();
		Term sysTerm = DpUserInfo.getDpUserInfo().getSysManage().getSysTerm();
		List<Date> refDateList = createDateList(sysYear, sysTerm);
		//mod Start 2022/5/11 H.Futagami バックログNo.13　過去実績参照で選択している品目を一覧に表示する
		//DeliveryResultInsListDto result = new DeliveryResultInsListDto(null, refDateList, jgiName, shokushuName);
		DeliveryResultInsListDto result = new DeliveryResultInsListDto(null, refDateList, jgiName, shokushuName, null);
		//mod End 2022/5/11 H.Futagami バックログNo.13　過去実績参照で選択している品目を一覧に表示する
		return result;
	}
