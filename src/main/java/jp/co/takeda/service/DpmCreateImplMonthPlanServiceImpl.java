 ...
 */
@Transactional
@Service("dpmCreateImplMonthPlanService")
public class DpmCreateImplMonthPlanServiceImpl implements DpmCreateImplMonthPlanService {

	/**
	 * LOGGER
	 */
	private static final Log LOG = LogFactory.getLog(DpmCreateImplMonthPlanServiceImpl.class);
	...
	// 実施計画を作成
	public ImplMonthPlan getImplMonthPlan(String prodCode, Long value, InsType insType, ValueChangeType valueChangeType, String tmsTytenCd, ImplMonthPlan implMonthPlan) throws LogicalException {
		if (LOG.isDebugEnabled()) {
			...
			LOG.debug("implPlan:" + implMonthPlan);
		}
		...
		// --------------
		// 更新前の実施計画
		ImplMonthPlan oriImplPlan = null;
		if (implMonthPlan == null) {
			oriImplPlan = new ImplMonthPlan();
		} else {
			oriImplPlan = implMonthPlan;
		}

		// 更新後の実施計画
		ImplMonthPlan result = new ImplMonthPlan();
