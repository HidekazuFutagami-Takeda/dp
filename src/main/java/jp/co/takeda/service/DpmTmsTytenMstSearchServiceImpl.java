...
import jp.co.takeda.logic.TmsTytenTenkaiListRealSearchLogic;
 ...
 */
@Transactional
@Service("dpmTmsTytenMstSearchService")
public class DpmTmsTytenMstSearchServiceImpl implements DpmTmsTytenMstSearchService {
	...
	public SosListDto search() throws LogicalException {
        ...
        List<String> ryutsuStringList = new ArrayList<String>();

        for(DpmCCdMst dpmCCdMst: ryutsuList) {
            ryutsuStringList.add(dpmCCdMst.getDataCd());
        }

        // 検索実行
		List<SosMst> resultList;
		try {
			resultList = sosMstRealDao.searchTokuyakutenbuList(ryutsuStringList);
	...
	public TmsTytenMstHontenListDto search(TmsTytenMstScDto scDto) throws LogicalException {
		...
		try {
			resultList = tmsTytenMstUnRealDao.searchRealHontenList(TmsTytenMstUnRealDao.SORT_STRING, scDto);
		} catch (DataNotFoundException e) {
	...
	public TmsTytenMstTenkaiListDto search(String hontenTmsTytenCd, String shishaTmsTytenCd, TmsTytenMstScDto scDto) throws LogicalException {
		...
		// 検索ロジック実行
		TmsTytenTenkaiListRealSearchLogic logic = new TmsTytenTenkaiListRealSearchLogic(tmsTytenMstUnRealDao, tmsTytenMstUn, scDto);
	...
	public TmsTytenMstResultsListDto searchResultsList(String sosCd, TmsTytenMstScDto scDto) throws LogicalException {
		...
		try {
			resultList = tmsTytenMstUnRealDao.searchRealResultsList(TmsTytenMstUnRealDao.SORT_STRING, sosCd, scDto);
		} catch (DataNotFoundException e) {
			throw new LogicalException(new Conveyance(new MessageKey("DPC1023E")), e);
		}
	...
	private String getSosName(String sosCd2) {
		...
		if (StringUtils.isNotEmpty(sosCd2)) {
			try {
				SosMst sosMst = sosMstRealDao.searchReal(sosCd2);
	...
	private String getSosCode(String addrCodePref) {
		...
		String sosCode = null;
		try {
			SosMst sosMst = sosMstRealDao.searchBumon(addrCodePref);
	...
	private TmsTytenMstUn getTmsTyten(String tmsTytenCd) {
		try {
			return tmsTytenMstUnRealDao.searchRealRef(tmsTytenCd);
		} catch (DataNotFoundException e) {
	...
	private List<TmsTytenMstUn> searchExecute(TmsTytenMstScDto scDto) throws LogicalException {
		List<TmsTytenMstUn> resultList;
		try {
			resultList = tmsTytenMstUnRealDao.searchRealList(TmsTytenMstUnRealDao.SORT_STRING, scDto);
		} catch (DataNotFoundException e) {
			throw new LogicalException(new Conveyance(new MessageKey("DPC1023E")), e);
		}
		return resultList;
