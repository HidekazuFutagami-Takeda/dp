 ...
 */
@Transactional
@Service("dpmBranchMonthPlanService")
public class DpmBranchMonthPlanServiceImpl implements DpmBranchMonthPlanService {
	...
	// 支店別月別計画登録・更新
	public int updateBranchPlan(String pgId, List<SosMonthPlanUpdateDto> updateDtoList) {
		...
		for (SosMonthPlanUpdateDto updateDto : updateDtoList) {
			...
			// 更新対象情報を作成
			ManageBranchMonthPlan branchPlan = new ManageBranchMonthPlan();
			...
			branchPlan.setDelFlg(Boolean.FALSE);
			branchPlan.setUpDate(updateDto.getUpDate());

			// 登録/更新
			try {
				if (branchPlan.getSeqKey() == null) {
					manageBranchMonthPlanDao.insert(branchPlan, pgId);
				} else {
					manageBranchMonthPlanDao.update(branchPlan, pgId);
				}
			} catch (DuplicateException e) {
				throw new SystemException(new Conveyance(ErrMessageKey.DB_DUPLICATE_ERROR, "支店別月別計画登録に失敗"), e);
			}
