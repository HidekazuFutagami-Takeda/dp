 ...
 */
@Transactional
@Service("dpmBranchPlanService")
public class DpmBranchPlanServiceImpl implements DpmBranchPlanService {
	...
	// 支店別計画登録・更新
	public ManagePlanUpdateResultDto updateBranchPlan(String pgId, List<SosPlanUpdateDto> updateDtoList) {
		...
		for (SosPlanUpdateDto updateDto : updateDtoList) {
			...
			final String sosCd2 = updateDto.getSosCd();
			...
			try {
				ManageBranchPlan branchPlan = manageBranchPlanDao.searchUk(insType, prodCode, sosCd2);
				orgImplPlan = branchPlan.getImplPlan();
			...
			// 更新対象情報を作成
			ManageBranchPlan branchPlan = new ManageBranchPlan();
			branchPlan.setSeqKey(updateDto.getSeqKey());
			branchPlan.setInsType(insType);
			branchPlan.setProdCode(prodCode);
			branchPlan.setSosCd(sosCd2);
			branchPlan.setImplPlan(newImplPlan);
			branchPlan.setDelFlg(isDelete(updateDto));
			branchPlan.setUpDate(updateDto.getUpDate());

			// 登録/更新
			try {
				if (branchPlan.getSeqKey() == null) {
					manageBranchPlanDao.insert(branchPlan, pgId);
				} else {
					manageBranchPlanDao.update(branchPlan, pgId);
				}
