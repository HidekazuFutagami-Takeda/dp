 ...
 */
@Transactional
@Service("dpmIyakuPlanService")
public class DpmIyakuPlanServiceImpl implements DpmIyakuPlanService {
	...
	// 全社計画登録・更新
	public ManagePlanUpdateResultDto updateIyakuPlan(String pgId, List<SosPlanUpdateDto> updateDtoList) {
		...
		for (SosPlanUpdateDto updateDto : updateDtoList) {
			...
			try {
				ManageIyakuPlan iyakuPlan = manageIyakuPlanDao.searchUk(insType, prodCode);
				orgImplPlan = iyakuPlan.getImplPlan();
			...
			// 更新対象情報を作成
			ManageIyakuPlan iyakuPlan = new ManageIyakuPlan();
			iyakuPlan.setSeqKey(updateDto.getSeqKey());
			iyakuPlan.setInsType(insType);
			iyakuPlan.setProdCode(prodCode);
			iyakuPlan.setImplPlan(newImplPlan);
			iyakuPlan.setDelFlg(isDelete(updateDto));
			iyakuPlan.setUpDate(updateDto.getUpDate());

			// 登録/更新
			try {
				if (iyakuPlan.getSeqKey() == null) {
					manageIyakuPlanDao.insert(iyakuPlan, pgId);
				} else {
					manageIyakuPlanDao.update(iyakuPlan, pgId);
				}
