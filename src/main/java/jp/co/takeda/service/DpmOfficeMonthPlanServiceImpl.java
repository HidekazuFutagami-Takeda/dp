 ...
 */
@Transactional
@Service("dpmOfficeMonthPlanService")
public class DpmOfficeMonthPlanServiceImpl implements DpmOfficeMonthPlanService {
	...
	// 営業所別月別計画登録・更新
	public int updateOfficePlan(String pgId, List<SosMonthPlanUpdateDto> updateDtoList) {
		...
		for (SosMonthPlanUpdateDto updateDto : updateDtoList) {
			...
			// 更新対象情報を作成
			ManageOfficeMonthPlan officePlan = new ManageOfficeMonthPlan();
			...
			officePlan.setDelFlg(Boolean.FALSE);
			officePlan.setUpDate(updateDto.getUpDate());

			// 登録/更新
			try {
				if (officePlan.getSeqKey() == null) {
					manageOfficeMonthPlanDao.insert(officePlan, pgId);
				} else {
					manageOfficeMonthPlanDao.update(officePlan, pgId);
				}
			} catch (DuplicateException e) {
				throw new SystemException(new Conveyance(ErrMessageKey.DB_DUPLICATE_ERROR, "営業所別月別計画登録に失敗"), e);
			}
