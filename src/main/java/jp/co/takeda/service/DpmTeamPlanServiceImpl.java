...
/**
 * 管理の組織別計画(チーム)の更新に関するサービス実装クラス
 *
 * @author nozaki
 */
@Transactional
@Service("dpmTeamPlanService")
public class DpmTeamPlanServiceImpl implements DpmTeamPlanService {
	...
	// チーム別計画登録・更新
	public ManagePlanUpdateResultDto updateTeamPlan(String pgId, List<SosPlanUpdateDto> updateDtoList) {
		...
		for (SosPlanUpdateDto updateDto : updateDtoList) {
			...
			if (updateDto.getSosCd() == null) {
				final String errMsg = "更新対象の組織コード(チーム)がnull";
				throw new SystemException(new Conveyance(PARAMETER_ERROR, errMsg));
			}
			...
			final String sosCd4 = updateDto.getSosCd();
			...
			try {
				ManageTeamPlan teamPlan = manageTeamPlanDao.searchUk(insType, prodCode, sosCd4);
				orgImplPlan = teamPlan.getImplPlan();
			...
			// 更新対象情報を作成
			ManageTeamPlan teamPlan = new ManageTeamPlan();
			teamPlan.setSeqKey(updateDto.getSeqKey());
			teamPlan.setEtcSosFlg(updateDto.getEtcSosFlg());
			teamPlan.setInsType(insType);
			teamPlan.setProdCode(prodCode);
			teamPlan.setSosCd(sosCd4);
			teamPlan.setImplPlan(newImplPlan);
			teamPlan.setDelFlg(isDelete(updateDto));
			teamPlan.setUpDate(updateDto.getUpDate());

			// 登録/更新
			try {
				if (teamPlan.getSeqKey() == null) {
					manageTeamPlanDao.insert(teamPlan, pgId);
				} else {
					manageTeamPlanDao.update(teamPlan, pgId);
				}
			} catch (DuplicateException e) {
				throw new SystemException(new Conveyance(DB_DUPLICATE_ERROR, "チーム別計画登録に失敗"), e);
			}
