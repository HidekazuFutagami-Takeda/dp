...
/**
 * 組織カテゴリコード検索サービス実装クラス
 * @author rna8405
 *
 */
@Transactional
@Service("dpmSosCtgSearchService")
public class DpmSosCtgSearchServiceImpl implements DpmSosCtgSearchService {
	...
	public List<SosCtg> searchDpmSosCtgList(String sosCd, String screenId) {
		...
		try {
			sosCtgList = dpmCViSosCtgDao.search(sosCd);
        ...
        for(JknGrp tmpJknGrp:user.getJknGrpList()) {
            if(screenId.toUpperCase().substring(0, 5).equals(tmpJknGrp.getJknGrpId().getDbValue())) {
                jknGrp = tmpJknGrp;
                break;
