...
/**
 * 組織別月別計画の検索結果 明細行を表すDTO
 *
 * @author stakeuchi
 */
public class SosMonthPlanResultDetailDto extends DpDto {
...
//add End 2022/08/25 R.takamoto No.8　計画管理の月別計画に納入実績の値を表示
	 ...
	 * コンストラクタ(支店別月別計画)
	 *
	 * @param branchPlan 支店別月別計画
	 * @param canMovePlanLevel プランレベル移行可否フラグ
	 */
	public SosMonthPlanResultDetailDto(ManageBranchMonthPlan branchPlan, boolean canMovePlanLevel) {
		this.sosCd = branchPlan.getSosCd();
		this.sosName = branchPlan.getBumonSeiName();
		this.jgiNo = null;
		this.jgiName = null;
...
//add End 2022/08/25 R.takamoto No.8　計画管理の月別計画に納入実績の値を表示
	 ...
	 * コンストラクタ(営業所別月別計画)
	 *
	 * @param officePlan 営業所別月別計画
	 * @param canMovePlanLevel プランレベル移行可否フラグ
	 */
	public SosMonthPlanResultDetailDto(ManageOfficeMonthPlan officePlan, boolean canMovePlanLevel) {
		this.sosCd = officePlan.getSosCd();
		this.sosName = officePlan.getBumonSeiName();
		this.jgiNo = null;
		this.jgiName = null;
...
//add End 2022/08/25 R.takamoto No.8　計画管理の月別計画に納入実績の値を表示
	 ...
	 * コンストラクタ(担当者別月別計画)
	 *
	 * @param mrPlan 担当者別月別計画
	 * @param canMovePlanLevel プランレベル移行可否フラグ
	 * @param etcSosFlg 雑組織フラグ
	 */
	public SosMonthPlanResultDetailDto(ManageMrMonthPlan mrPlan, boolean canMovePlanLevel,
			boolean etcSosFlg) {
		this.sosCd = null;
		this.sosName = null;
		this.jgiNo = mrPlan.getJgiNo();
		this.jgiName = mrPlan.getJgiName();
...
//add Start 2022/08/25 R.takamoto No.8　計画管理の月別計画に納入実績の値を表示
			this.record1ValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecord1ValueYb());
			this.record2ValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecord2ValueYb());
			this.record3ValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecord3ValueYb());
			this.record4ValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecord4ValueYb());
			this.record5ValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecord5ValueYb());
			this.record6ValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecord6ValueYb());
			this.recordTotalValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecordTotalValueYb());
			this.recordTougetuValueYb = ConvertUtil.parseMoneyToThousandUnitDouble(mrPlan.getImplMonthPlan().getRecordTougetuValueYb());
//add End 2022/08/25 R.takamoto No.8　計画管理の月別計画に納入実績の値を表示
